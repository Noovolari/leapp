...
export default class StartSsmSession extends LeappCommand {
  ...
  async startSsmSession(credentials: CredentialsInfo, ssmInstanceIdString: string, r: string): Promise<void> {
    ...
    if (process.platform === "darwin") {
      ...
      macOsTerminalType = terminalProgram && terminalProgram.toLowerCase().includes("iterm") ? constants.macOsIterm2 : terminalProgram.toLowerCase().includes("warp") ? constants.macOsWarp : constants.macOsTerminal;
    }
