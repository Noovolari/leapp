...
class PluginService extends service_1.Service {
    ...
    async listPlugin(queryParam) {
        const pluginListRds = (await this.dataApiConnector.executeStatement(`SELECT p.*, a.id a_id, a.name AS a_name, a.surname AS a_surname, a.email AS a_email, s.id AS s_id, s.name AS s_name FROM plugin AS p
    ...
    async getPlugin(pluginName) {
        const pluginRds = (await this.dataApiConnector.executeStatement(`SELECT p.*, a.id a_id, a.name AS a_name, a.surname AS a_surname, a.email AS a_email, s.id AS s_id, s.name AS s_name FROM plugin AS p
            JOIN author a ON p.author_id = a.id
            JOIN status s ON p.status_id = s.id
            WHERE p.plugin_name = :id::uuid`, {
            name: pluginName
        })).records[0];
        if (!pluginRds) {
