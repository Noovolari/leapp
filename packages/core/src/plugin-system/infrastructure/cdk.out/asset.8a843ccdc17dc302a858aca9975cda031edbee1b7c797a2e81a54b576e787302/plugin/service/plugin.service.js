...
class PluginService extends service_1.Service {
    ...
    async listPlugin(queryParam) {
        const pluginListRds = (await this.dataApiConnector.executeStatement(`SELECT p.*, a.id, a.name, a.surname, a.email, s.id, s.name FROM plugin AS p
    ...
    async getPlugin(pluginName) {
        const pluginRds = (await this.dataApiConnector.executeStatement(`SELECT p.*, a.id, a.name, a.surname, a.email, s.id, s.name FROM plugin AS p
            JOIN author a ON p.author_id = a.id
            JOIN status s ON p.status_id = s.id
