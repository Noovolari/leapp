...
class PluginService extends service_1.Service {
    ...
    async listPlugin(queryParam) {
        const pluginListRds = (await this.dataApiConnector.executeStatement(`SELECT p.*, a.*, s.* FROM plugin AS p
    ...
    async getPlugin(pluginId) {
        const pluginRds = (await this.dataApiConnector.executeStatement(`SELECT p.*, a.*, s.* FROM plugin AS p
            JOIN author a ON p.author_id = a.id
            JOIN status s ON p.status_id = s.id
            WHERE p.id = :id`, {
            id: pluginId
