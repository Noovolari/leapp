// Used to define app data initial configuration
export const initialConfiguration = {
  licence: '',            // maybe used in the future to define different types of licences
  uid: '',                // unique identifier of the current installation
  language: 'en',         // current UI language
  defaultWorkspace: '',   // current workspace
  avatar: '',             // if needed in the future
  name: 'default',
  workspaces: []          // collection of workspaces which define the session of login and their properties
};
